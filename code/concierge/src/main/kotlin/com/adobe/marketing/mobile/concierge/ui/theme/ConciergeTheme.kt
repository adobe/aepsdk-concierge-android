...
/**
 * Copies all 15 Material3 typography slots with the given [fontFamily] and an optional
 * [lineHeightMultiplier] applied relative to each slot's font size.
 */
...
fun ConciergeTheme(
    ...
    CompositionLocalProvider(LocalActiveConciergeTheme provides activeTheme) {
        if (resolvedFamily != null) {
            ...
            val themed = buildThemedTypography(base, resolvedFamily)
            MaterialTheme(
                ...
                CompositionLocalProvider(
                    LocalTextStyle provides LocalTextStyle.current.copy(
                        fontFamily = resolvedFamily ?: LocalTextStyle.current.fontFamily
                    ),
                    content = content
                )
            }
